...
t_control <- caret::trainControl(method = "cv",number = 2) ### I WILL USE 5
### LATER.
...
### NOTE: Double brackets were used to access the first element of the     
...
for (drug in all_drugs) {
  ...
  ###  NOTE: metrics_CNV[[1]]$RF[1] can be used to directly access RF method
  ### RMSE value for the first drug. $ operator should be used more for the
  ### sake of easy readability.
...
for (drug in all_drugs) {
  ...
  best <-  metrics_CNV %>% as.data.frame() %>% select(starts_with(drug)) %>% 
    t() %>% as.data.frame() %>% select(RMSE)
...
summary_DF_CNV %>% kableExtra::kbl(caption = "BEST MODELS - CNV") %>% kableExtra::kable_classic(full_width = F,html_font = "Cambria", 
font_size = 20) %>% 
  kableExtra::save_kable(file = paste0("BEST_MODELS_CNV",".png"))   
...
### NOTE: Both CNV and gCSI test and holdout data frames have same samples ,
### respectively.
...
for (i in c(1:length(my_model_list_GEXP))) { ### 
...
### Best method by the lowest RMSE for each drug:
 
### GEXP:
 
 
 best_model_GCSI_GEXP <- c() ### Best method for each drug will be stored.
 
 best_model_GCSI_CNV <- c() ### same for the CNV data.
 
 min_RMSE_gcsi_gexp <- c()   ### The RMESE value for the best method for each
 ####drug will be stored.
 
 min_RMSE_gcs_cnv <- c()   ### same for the CNV data.
 
for (i in c(1:length(all_drugs))) { ### For all 11 drugs:
  

### I convert current drug's list into a data.frame. In this data.frame,
### first row contains RMSE values for each method in the respective columns.
### I retrieve the minimum of them and add to the vector.
  
    
min_RMSE_gcsi_gexp <- c(min_RMSE_gcsi_gexp, (metrics_gCSI_test_gexp[[i]] %>%
                                      as.data.frame() %>% .[1,] %>% min))
 

### I do the same above but this time I retrieve the position of the smallest
### RMASE value in the data frame by which.min function, which gives the 
### position as a named integer vector. I then take its name and add the 
### vector. Thereby, I acquired the name of the regressor method with the
### minimum error.


best_model_GCSI_GEXP  <- c(best_model_GCSI_GEXP,(metrics_gCSI_test_gexp[[i]] %>% 
                        as.data.frame() %>% .[1,] %>% which.min() %>% names))

### Get the min RMSE value for CNV data:
    
min_RMSE_gcs_cnv <- c(min_RMSE_gcs_cnv, (metrics_gCSI_test_CNV[[i]] %>%
                                      as.data.frame() %>% .[1,] %>% min)) 


### Get the best method:

best_model_GCSI_CNV <- c(best_model_GCSI_CNV,(metrics_gCSI_test_CNV[[i]] %>% 
                        as.data.frame() %>% .[1,] %>% which.min() %>% names))
...
summary_DF_GCSI_GEXP <- tibble(drugs=all_drugs, RMSE=min_RMSE_gcsi_gexp, 
                                                  model=best_model_GCSI_GEXP)

print(summary_DF_GCSI_GEXP)

summary_DF_GCSI_GEXP %>% kableExtra::kbl(caption = "BEST-MODELS-GCSI-GEXP") %>% kableExtra::kable_classic(full_width = F,html_font = "Cambria", 
font_size = 20) %>% 
  kableExtra::save_kable(file = paste0("BEST_MODELS_GCSI_GEXP",".png"))   


 
summary_DF_GCSI_CNV <- tibble(drugs=all_drugs, RMSE=min_RMSE_gcs_cnv, 
                                                  model=best_model_GCSI_CNV)

print(summary_DF_GCSI_CNV)

summary_DF_GCSI_CNV %>% kableExtra::kbl(caption = "BEST-MODELS-GCSI-CNV") %>% kableExtra::kable_classic(full_width = F,html_font = "Cambria", 
font_size = 20) %>% 
  kableExtra::save_kable(file = paste0("BEST_MODELS_GCSI_CNV",".png"))   


### I will apply the best models for each drug on the GCSI holdout datasets:
...
### I need to subset also the gCSI drug data frame since it contains the
###patients from the both test and the holdout groups. I will subset 
### it to the patients who are used in the gCSI test set.

gCSI_drugs_holdout <- gCSI_drugs[,row.names(t_gCSI_holdout_gex)]
 

### prepare the data before the evaluation of the model:

### Firstly, I will need to subset the data to the used 1000 genes.

###

 my_gCSI_HOLDOUT_list_gexp <- c() ### I will store the subset of datasets 
                                  ### in these empty vectors.
 
 my_gCSI_HOLDOUT_list_CNV <- c()
 

 
for (i in 1:nrow( gCSI_drugs_holdout )) {
 
 ### the following function subsets the dataset for each drug by using the
 ### previously created 1000 highest variable genes used for each drug in
 ### the generation of each model. We do this because test datasets should
 ### have same features with the train data used in the modelling process.
   
 GEX_prep_gCSI_H <- prepare_data_for_GCSI(M = t_gCSI_holdout_gex, 
            drugs = gCSI_drugs_holdout, 
            drugName = row.names(gCSI_drugs_holdout)[i],
            top1000 = TRUE,GEXP = TRUE)
 
 CNV_prep_gCSI_H <-  prepare_data_for_GCSI(M = t_gCSI_holdout_cnv, 
            drugs = gCSI_drugs_holdout, 
            drugName = row.names(gCSI_drugs_holdout)[i],
            top1000 = TRUE, CNV = TRUE)
 
 
 name_gexp_gCSI_H <- row.names(gCSI_drugs_holdout)[i]
 
 my_gCSI_HOLDOUT_list_gexp[[ name_gexp_gCSI_H ]] <- GEX_prep_gCSI_H
 
 name_CNV_gCSI_H <- row.names(gCSI_drugs_holdout)[i]
 
 my_gCSI_HOLDOUT_list_CNV[[ name_CNV_gCSI_H ]] <-  CNV_prep_gCSI_H
 
}

### I check the dimensions :

 #sapply( my_gCSI_HOLDOUT_list_CNV, dim)
 #sapply( my_gCSI_HOLDOUT_list_gexp, dim)
...
### I will need to use the best models for each drug in the model fitting
### function.


### I will change the names of the lists that contain the model objects 
### for each drug, from "Crizotinib_train" to "Crizotinib" etc. 
### So, I can easily access by drug names.

names(my_model_list_GEXP) <- all_drugs
names(my_model_list_CNV) <- all_drugs

 
best_model_GCSI_GEXP %>% print()  ### contains the best models for each drug. 
best_model_GCSI_CNV %>% print()   ### contains the best models for each drug.

 
### I will also add the drug names to each element corresponding to the
### respective drugs to be able to access by supplying drug names.

names(best_model_GCSI_GEXP) <- all_drugs
names(best_model_GCSI_CNV) <- all_drugs

best_gex <- best_model_GCSI_GEXP
best_cnv <- best_model_GCSI_CNV

metrics_HOLDOUT_gexp <- c()
metrics_HOLDOUT_CNV <- c()
 

for (drug in all_drugs) { ### 
  

    metrics_HOLDOUT_gexp[[drug]] <- 
        test_model(model_fit = my_model_list_GEXP[[drug]][[best_gex[drug]]],
                                  test_data = my_gCSI_HOLDOUT_list_gexp[[drug]])
  
    metrics_HOLDOUT_CNV[[drug]] <- 
        test_model(model_fit = my_model_list_CNV[[drug]][[best_cnv[drug]]],
                                test_data = my_gCSI_HOLDOUT_list_CNV[[drug]])
  
}

### I will retrieve the RMSE values into a vector for both GEXP and CNV:
...
RMSE_gexp <- c(RMSE_gexp, metrics_HOLDOUT_gexp[[drug]][[1]])

RMSE_CNV <- c(RMSE_CNV, metrics_HOLDOUT_CNV[[drug]][[1]])   
  
}


summary_tibble_HOLDOUT_gexp <- tibble("DRUGS"=all_drugs, 
                                         "RMSE"= RMSE_gexp,"method"=best_gex)

summary_tibble_HOLDOUT_CNV <- tibble("DRUGS"=all_drugs,
                                         "RMSE"= RMSE_CNV, "method"=best_cnv)
...
### I print the tables for the previously generated summary tibbles:

summary_DF_GCSI_GEXP %>% kableExtra::kbl(caption = "GEXP_GDSC_TEST_METRICS") %>% kableExtra::kable_classic(full_width = F,html_font = "Cambria", 
font_size = 20) %>% kableExtra::save_kable(file = paste0("./SUMMARY_OF_THE_RESULTS/SUMMARY_GDSC_TEST_GEXP",".png")) 


summary_DF_GCSI_CNV %>% kableExtra::kbl(caption = "CNV_GDSC_TEST_METRICS") %>% kableExtra::kable_classic(full_width = F,html_font = "Cambria", 
font_size = 20) %>% kableExtra::save_kable(file = paste0("./SUMMARY_OF_THE_RESULTS/SUMMARY_GDSC_TEST_CNV",".png")) 



# summary results for each drug from cross-dataset-prediction (gCSI -> holdout)

summary_tibble_HOLDOUT_gexp %>% kableExtra::kbl(caption = "GEXP_gCSI_HOLDOUT_METRICS") %>% kableExtra::kable_classic(full_width = F,html_font = "Cambria", font_size = 20) %>% 
kableExtra::save_kable(file = paste0("./SUMMARY_OF_THE_RESULTS/SUMMARY_GEXP_gCSI_HOLDOUT",".png")) 


summary_tibble_HOLDOUT_CNV %>% kableExtra::kbl(caption = "CNV_gCSI_HOLDOUT_METRICS") %>% kableExtra::kable_classic(full_width = F,html_font = "Cambria", 
font_size = 20) %>% kableExtra::save_kable(file = paste0("./SUMMARY_OF_THE_RESULTS/SUMMARY_CNV_gCSI_HOLDOUT",".png")) 



### correlation coefficients can also be printed as an alternative.
